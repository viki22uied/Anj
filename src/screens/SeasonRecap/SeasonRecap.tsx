...
export default function SeasonRecap({ onContinue }: { onContinue: () => void }) {
  ...
  const lessons = useMemo(() => {
    ...
    // What could have been logic
    const moneylenderInterestPaid = gameState.debtBreakdown
      .filter(d => d.lender === 'moneylender')
      .reduce((sum, d) => sum + (d.amount - (d.principal || d.amount)), 0);

    if (moneylenderInterestPaid > 0 && gameState.hasKCC) {
      l.push(isHi ? `एक जानकारी: इस सीज़न साहूकार को ब्याज दिया: ${fmt(moneylenderInterestPaid)}। KCC इस्तेमाल करते तो बहुत कम लगता। फर्क: ${toRealLifeEquivalent(moneylenderInterestPaid, farmer.state, isHi)}` : `Insight: You paid ${fmt(moneylenderInterestPaid)} in interest to the moneylender. KCC would have been much cheaper. Difference: Equivalent to ${toRealLifeEquivalent(moneylenderInterestPaid, farmer.state, isHi)}`);
    } else if (gameState.debtBreakdown.some(d => d.lender === 'moneylender')) {
      l.push(isHi ? `🦈 साहूकार का कर्ज़ भारी है। Bank Mitra से बात करके KCC लें।` : `🦈 Informal loans are expensive. Switch to KCC through a Bank Mitra.`);
    }

    if (!gameState.pmfbyEnrolled && totalLoss > 0) {
      // PRD: Compare PMFBY cost vs Loss
      l.push(isHi ? `एक जानकारी: इस सीज़न नुकसान हुआ: ${fmt(totalLoss)}। PMFBY प्रीमियम 2% था। बीमा होता तो नुकसान नहीं होता।` : `Insight: You lost ${fmt(totalLoss)}. PMFBY premium was 2%. Insurance would have covered the loss.`);
    }
    
    if (gameState.eNWRActive) {
      l.push(isHi ? `✅ गोदाम (WDRA) का सही इस्तेमाल! आपने distress sale से खुद को बचाया।` : `✅ Good use of Godown (eNWR)! You avoided a distress sale.`);
    }

    if (l.length === 0) l.push(isHi ? '📈 बहुत बढ़िया! आपने इस सीज़न सही फैसले लिए।' : '📈 Great job! You made solid choices this season.');
    return l;
  }, [gameState, farmer.state, totalLoss, isHi]);
  ...
  const showFamilyQuestion = gameState.netIncome < 0 || totalLoss > 5000;
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        {/* PRD Family Question Component */}
        {showFamilyQuestion && (
           <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1 }}
             className="mt-6 bg-red-50 p-5 rounded-2xl border border-red-100 flex gap-4">
             <div className="text-4xl">👩🏽‍🌾</div>
             <div>
               <div className="text-[10px] font-bold text-red-500 uppercase tracking-widest mb-1">{isHi ? 'परिवार' : 'Family'}</div>
               <p className="font-bold text-slate-800 leading-tight">{isHi ? '"पिछली बार बहुत नुकसान हुआ था। इस बार क्या करोगे अलग?"' : '"We had huge losses last time. What will you do differently now?"'}</p>
             </div>
           </motion.div>
        )}
