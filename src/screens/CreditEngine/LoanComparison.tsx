  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8 relative">
      <div className="px-5 pt-6 pb-4">
        ...
        <h1 className="font-display text-3xl text-slate-800">{isHi ? 'उधार लो / Take Loan' : 'Take Loan'}</h1>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="bg-red-50 rounded-3xl p-5 shadow-sm border border-red-100 relative overflow-hidden">
          ...
          <h2 className="font-display text-2xl text-red-800 relative z-10 mb-1">{isHi ? 'साहूकार / Moneylender' : 'Moneylender'}</h2>
          ...
          <div className="relative z-10">
            {!gameState.hasKCC ? (
              gameState.kccApplicationPending ? (
                gameState.kccApprovalWeeksRemaining > 0 ? (
                  ...
                  <div className="bg-emerald-50 p-4 rounded-xl shadow-sm border border-emerald-200">
                    <div className="flex gap-4">
                      ...
                      <div>
                        <div className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest mb-1">Bank Mitra</div>
                        <p className="font-bold text-emerald-800 text-sm mb-3">
                          {isHi ? '"नमस्ते! आपका KCC approved हो गया है। कोई ज़मानत नहीं। कोई छिपी फीस नहीं। यह आपका हक है।"' : '"Namaste! Your KCC is approved. No collateral. No hidden fees. It is your right."'}
                        </p>
                        <button onClick={() => useGameStore.getState().claimKCC()} className="w-full bg-emerald-600 text-white py-3 rounded-lg shadow-md font-bold text-sm tracking-wide hover:bg-emerald-700">
                          {isHi ? 'KCC क्लेम करें' : 'Claim KCC'}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200">
          <h2 className="font-display text-xl text-slate-800 mb-1">{isHi ? 'Cooperative (सहकारी)' : 'Cooperative Society'}</h2>
