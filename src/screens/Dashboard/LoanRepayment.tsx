  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8 relative">
      <div className="px-5 pt-6 pb-4">
        ...
        <h1 className="font-display text-3xl text-slate-800">{isHi ? 'कर्ज़ चुकाओ / Repay Debt' : 'Repay Debt'}</h1>
        <div className="flex justify-between items-center mt-3 bg-white p-3 rounded-xl shadow-sm border border-slate-100">
          <div>
            ...
            <div className={`font-number text-xl font-black ${gameState.totalDebt > 0 ? 'text-red-500' : 'text-slate-600'}`}>
          ...
          <div className="text-right">
            ...
            <div className="font-number text-xl font-black text-emerald-600">
      ...
      <div className="p-4 space-y-4">
        ...
        {gameState.kccUsed > 0 && (
          <button onClick={repayAllKCC} disabled={gameState.cashInHand < gameState.kccUsed}
            className={`w-full p-4 rounded-3xl shadow-sm flex items-center justify-center gap-2 border font-bold uppercase tracking-widest text-xs transition-colors
             ...
             {isHi ? 'सब KCC चुकाओ / Pay All KCC' : 'Pay All KCC'}
          </button>
        )}
              ...
              <button onClick={() => setExpandedId(isExpanded ? null : debt.id)}
                ...
                <div className="text-left">
                  <div className={`font-bold text-lg text-${baseColor}-800`}>
                    {isML ? (isHi ? 'साहूकार' : 'Moneylender') : isKCC ? 'KCC' : debt.lender === 'cooperative' ? (isHi ? 'सहकारी' : 'Cooperative') : debt.lender === 'enwr_pledge' ? 'eNWR Pledge' : 'NBFC'}
                  </div>
                ...
                {isExpanded && (
                  <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
                    ...
                    <button onClick={() => repayLoan(debt.id, debt.amount)} disabled={gameState.cashInHand < debt.amount}
                      ...
                      {isHi ? 'पूरा चुकाओ / Pay Full' : 'Pay Full Amount'}
                    </button>
        ...
        {gameState.debtBreakdown.length === 0 && (
          <div className="bg-emerald-50 rounded-3xl p-8 shadow-sm border border-emerald-100 text-center">
             ...
             <h2 className="font-display text-2xl text-emerald-800">{isHi ? 'कर्ज़ मुक्त!' : 'Debt Free!'}</h2>
             <p className="text-sm font-medium text-emerald-600/80 mt-2 mt-1">{isHi ? 'आप पूरी तरह से कर्ज़ मुक्त हैं। बहुत बढ़िया!' : 'You are completely debt free. Great job!'}</p>
          </div>
        )}
