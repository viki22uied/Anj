  ...
  return (
     <div className="min-h-screen bg-slate-50 flex flex-col pt-8 pb-12 relative">
       <div className="px-5 mb-8">
        <h1 className="font-display text-4xl text-emerald-800 mb-2">{isHi ? 'कटाई का समय!' : 'Harvest Time!'}<br/>{isHi ? 'Harvest Time!' : ''}</h1>
        <p className="text-slate-600 font-medium">{isHi ? 'आपकी फसल तैयार है। आप फसल का क्या करना चाहते हैं?' : 'Your crop is ready. What do you want to do with the harvest?'}</p>
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        <button onClick={handleSellNow} className="w-full bg-emerald-600 text-white p-5 rounded-2xl shadow-md hover:bg-emerald-700 transition-colors flex items-center justify-between">
          <div className="text-left">
            <div className="font-bold text-xl uppercase tracking-wide flex items-center gap-2">
              <span className="text-2xl">💰</span> {isHi ? 'तुरंत बेचो' : 'Sell Now'} / {isHi ? 'Sell Now' : 'Sell Immediately'}
            </div>
            <div className="text-xs font-medium text-emerald-100 mt-1">{isHi ? 'नकद भुगतान, कम दाम, कोई जोखिम नहीं' : 'Instant cash, lower price, no storage risk'}</div>
          </div>
        ...
        <button onClick={handleWait} className="w-full bg-white border border-slate-200 text-slate-800 p-5 rounded-2xl shadow-sm hover:border-slate-300 transition-colors flex items-center justify-between">
          <div className="text-left">
            <div className="font-bold text-lg flex items-center gap-2">
              <span className="text-2xl">⏳</span> {isHi ? 'रुक जाओ / Wait' : 'Wait on Farm'}
            </div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mt-1">{isHi ? 'खेत पर रखें · 3-5% नुकसान का जोखिम' : 'Store on farm · Lose 3-5% to damage'}</div>
          </div>
        ...
        <button onClick={handleGodown} className="w-full bg-amber-50 border border-amber-200 text-amber-900 p-5 rounded-2xl shadow-sm hover:bg-amber-100 transition-colors flex items-center justify-between">
          <div className="text-left">
            <div className="font-bold text-lg flex items-center gap-2">
              <span className="text-2xl">🏛️</span> {isHi ? 'गोदाम में रखो / eNWR' : 'Store in Godown'}
            </div>
            <div className="text-[10px] font-bold uppercase tracking-widest text-amber-600 mt-1">{isHi ? 'सुरक्षित स्टोरेज · 70% लोन · बाद में अच्छा दाम' : 'Store safe · Get 70% loan · High price later'}</div>
          </div>
