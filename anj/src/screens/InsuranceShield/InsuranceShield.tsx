      ...
      <div className="p-4 space-y-4">
        ...
        <div className={`rounded-3xl p-6 border shadow-sm flex flex-col items-center text-center justify-center relative overflow-hidden ${enrolled ? 'bg-blue-600 border-blue-700' : 'bg-white border-slate-200'}`}>
          ...
          {enrolled ? (
            <>
              ...
              <h2 className="font-display text-2xl text-white mb-1 relative z-10">Protected</h2>
              <p className="text-blue-100 text-xs font-bold uppercase tracking-widest relative z-10">Up to {fmt(maxCover)} coverage active</p>
            </>
          ) : (
            <>
               <div className="text-6xl mb-3 relative z-10 grayscale opacity-60">🛡️</div>
              <h2 className="font-display text-2xl text-slate-800 mb-1 relative z-10">Not Protected</h2>
              <p className="text-slate-500 text-xs font-bold uppercase tracking-widest relative z-10">Crops are vulnerable to disasters</p>
            </>
          )}
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-slate-200">
           ...
           {enrolled && gameState.pmfbyClaimStatus === 'none' && (
             <>
                <h3 className="text-xs font-bold uppercase tracking-widest mb-4 text-slate-500 text-center flex items-center gap-2 justify-center">
                 <span>📱</span> File a Claim (दावा)
               </h3>
               <p className="text-sm font-medium text-slate-600 text-center mb-4">Did your crop suffer extreme weather damage? File a claim within 72 hours via crop cutting experiment (CCE).</p>
           ...
           {pending && (
             <div className="text-center bg-amber-50 border border-amber-200 p-4 rounded-xl">
               ...
               <h3 className="font-bold text-amber-800 text-sm">Claim Processing</h3>
               <p className="text-amber-600/80 text-xs mt-1">CCE survey is being processed. It will take {gameState.claimProcessingWeeksRemaining} more weeks.</p>
             </div>
           )}
           ...
           {claimable && (
             <div className="text-center bg-emerald-50 border border-emerald-200 p-4 rounded-xl">
               ...
               <h3 className="font-bold text-emerald-800 text-sm mb-1">Claim Approved!</h3>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
           <div className="font-bold text-sm mb-3 flex items-center gap-2 uppercase tracking-wide text-slate-700">
             ℹ️ What does PMFBY cover?
           </div>
           <ul className="space-y-2.5 text-xs text-slate-600 font-medium">
             <li className="flex items-start gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
               <span className="text-blue-500 mt-0.5">▸</span><span>Droughts, dry spells, and insufficient rain</span>
             </li>
             <li className="flex items-start gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
               <span className="text-blue-500 mt-0.5">▸</span><span>Floods, inundation, and landslides</span>
             </li>
             <li className="flex items-start gap-2 bg-slate-50 p-2 rounded-lg border border-slate-100">
               <span className="text-blue-500 mt-0.5">▸</span><span>Mass pest attacks and severe crop diseases</span>
             </li>
           </ul>
        </div>
