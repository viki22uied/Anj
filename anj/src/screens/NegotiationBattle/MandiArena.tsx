...
export default function MandiArena({ onBack }: { onBack: () => void }) {
  ...
  const [result, setResult] = useState<{ price: number; won: boolean; msp: number } | null>(null);
  ...
  const handleAction = useCallback((action: string) => {
    ...
    if (action === 'accept' || negotiation.round >= negotiation.maxRounds) {
      ...
      setResult({ price: finalPrice, won: finalPrice >= negotiation.mspPrice, msp: negotiation.mspPrice });
      return;
    }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 relative">
        ...
        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
          className="game-card text-center max-w-sm w-full relative z-10 p-8 shadow-xl border-t-4 border-amber-400 bg-white">
          <motion.div className="text-7xl mb-6" animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 0.5 }}>
            {result.won ? '🏆' : result.price === 0 ? '🚶' : '😔'}
          </motion.div>
          ...
          {result.price > 0 ? (
            <>
              <p className={`text-sm mb-4 font-bold ${result.won ? 'text-emerald-600' : 'text-red-500'}`}>
                {result.won ? t('negotiation.result_won', 'Great Deal!') : t('negotiation.result_lost', 'Below MSP.')}
              </p>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 pb-20 pt-16 relative">
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 gap-6 pt-10 relative">
        <motion.div animate={isArhatiTalking ? { scale: [1, 1.08, 1] } : {}}
          transition={{ repeat: isArhatiTalking ? Infinity : 0, duration: 0.3 }}
          className="text-[100px] drop-shadow-2xl z-10 mx-auto">
          🧔
        </motion.div>
