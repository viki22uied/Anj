  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-8 rounded-t-3xl border-t border-slate-200 mt-2">
      <div className="sticky top-0 bg-white/90 backdrop-blur z-10 px-5 py-4 flex items-center gap-3 border-b border-slate-100 shadow-sm">
        ...
        <h2 className="font-display text-2xl text-slate-800">Market Prices</h2>
      </div>

      <div className="p-5">
        <div className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 mb-6 flex flex-col items-center">
           <span className="text-[10px] uppercase font-bold text-slate-400 tracking-widest mb-1">Today's Price</span>
        ...
        <h3 className="text-[11px] font-bold uppercase tracking-widest text-slate-400 mb-3 px-1">Price Projection</h3>
        ...
        {isStressed ? (
          <div className="p-8 bg-red-50 border border-red-100 rounded-3xl text-center shadow-inner relative overflow-hidden">
             ...
             <div className="relative z-10">
               ...
               <h4 className="font-bold text-red-800 mb-1">Cannot Think Clearly</h4>
               <p className="text-xs text-red-600/80 leading-relaxed max-w-[200px] mx-auto">Your stress level is too high to analyze market predictions.</p>
             </div>
          </div>
        ) : (
          <div className="p-5 bg-white border border-slate-100 shadow-sm rounded-3xl">
            <div className="flex justify-between items-end mb-4">
              <span className="text-xs font-bold text-slate-500">Predicted trend (12 wks)</span>
            </div>
        ...
        <div className="mt-6 flex flex-col gap-3">
           ...
           <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm">
             <button onClick={() => setShowMSPInfo(!showMSPInfo)} className="w-full bg-slate-50 hover:bg-slate-100 transition-colors p-4 flex items-center justify-between">
                <span className="font-bold text-slate-600 text-sm flex items-center gap-2">📍 What is MSP?</span>
             ...
             <AnimatePresence>
               {showMSPInfo && (
                 <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="p-4 text-xs text-slate-500 leading-relaxed bg-white">
                   <p className="mb-2"><strong>Minimum Support Price (MSP)</strong> is the guaranteed price offered by the Government of India for your crop.</p>
                   <p>If the Mandi prices fall below this limit, you hold the right to sell your harvest at procurement centers to secure your income!</p>
                 </motion.div>
               )}
             </AnimatePresence>
           </div>
           ...
           <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-4">
             <div className="font-bold text-slate-600 text-sm flex items-center gap-2 mb-3">🔔 Set Price Alert</div>
             {!alertSet ? (
               <div className="flex gap-2">
                 <input type="number" value={alertValue} onChange={(e) => setAlertValue(e.target.value)} placeholder={`Target over ₹${gameState.currentMarketPrice}`} className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-brand" />
                 <button onClick={handleSetAlert} className="bg-amber-100 text-amber-800 px-4 rounded-xl font-bold text-sm border border-amber-200">Set</button>
               </div>
             ) : (
               <div className="text-emerald-600 text-xs font-bold flex items-center gap-1 bg-emerald-50 px-3 py-2 rounded-lg">✅ Alert Armed! We will notify you when price targets are hit.</div>
             )}
           </div>
        </div>
      </div>
