...
const LANGUAGES: { code: Language; native: string }[] = [
  { code: 'hi', native: 'हिन्दी' }, { code: 'ta', native: 'தமிழ்' },
  { code: 'te', native: 'తెలుగు' }, { code: 'bn', native: 'বাংলা' },
  { code: 'mr', native: 'मराठी' }, { code: 'gu', native: 'ગુજરાતી' },
  { code: 'kn', native: 'ಕನ್ನಡ' }, { code: 'pa', native: 'ਪੰਜਾਬੀ' },
  { code: 'or', native: 'ଓଡ଼ିଆ' }, { code: 'as', native: 'অসমীয়া' },
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'language' && (
          <motion.div key="lang" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} className="w-full max-w-md relative z-10">
            <h1 className="font-display text-5xl mb-3 text-center text-amber-600">अनाज अर्थ</h1>
        ...
        {step === 'state' && (
          <motion.div key="state" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-md relative z-10">
            <button onClick={() => setStep('language')} className="text-sm font-bold text-slate-400 mb-6 hover:text-amber-500 flex items-center gap-1">← वापस / Back</button>
            <h2 className="font-display text-3xl mb-6 text-slate-800">राज्य चुनें / Select State</h2>
        ...
        {step === 'crop' && (
          <motion.div key="crop" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-md relative z-10">
            <button onClick={() => setStep('state')} className="text-sm font-bold text-slate-400 mb-6 hover:text-amber-500 flex items-center gap-1">← Back</button>
            <h2 className="font-display text-3xl mb-6 text-slate-800">मुख्य फसल / Primary Crop</h2>
        ...
        {step === 'land' && (
          <motion.div key="land" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-md relative z-10">
            <button onClick={() => setStep('crop')} className="text-sm font-bold text-slate-400 mb-6 hover:text-amber-500 flex items-center gap-1">← Back</button>
            <h2 className="font-display text-3xl mb-6 text-slate-800">ज़मीन / Land Holding</h2>
            <div className="space-y-4">
              <button onClick={() => { setLand({ amount: 0.75, type: 'marginal', cash: 8000, debt: 35000 }); setStep('name'); }}
                ...
                <div className="font-display text-xl text-slate-800">&lt; 1 हेक्टेयर — सीमांत किसान</div>
              ...
              <button onClick={() => { setLand({ amount: 1.5, type: 'small', cash: 15000, debt: 55000 }); setStep('name'); }}
                ...
                <div className="font-display text-xl text-slate-800">1–2 हेक्टेयर — लघु किसान</div>
              ...
              <button onClick={() => { setLand({ amount: 3.0, type: 'medium', cash: 30000, debt: 80000 }); setStep('name'); }}
                ...
                <div className="font-display text-xl text-slate-800">2–5 हेक्टेयर — मध्यम किसान</div>
        ...
        {step === 'name' && (
          <motion.div key="name" initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="w-full max-w-md relative z-10">
            <button onClick={() => setStep('land')} className="text-sm font-bold text-slate-400 mb-6 hover:text-amber-500 flex items-center gap-1">← Back</button>
            <h2 className="font-display text-3xl mb-6 text-slate-800">आपका नाम / Your Name</h2>
